  ...
  const cardContent = (
    <article className={cn(evolutionClasses.card.base, "group cursor-pointer")}>
      ...
      <div className="aspect-[3/4] sm:aspect-[2/3] lg:aspect-[3/4] overflow-hidden relative bg-muted">
        ...
        {/* Evolution About logo/branding - positioned at bottom right to avoid overlap */}
        <div className="absolute bottom-1 right-1 sm:bottom-2 sm:right-2 bg-yellow-400/90 text-black text-xs px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full font-medium opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
          <span className="hidden sm:inline">Evolution About</span>
          <span className="sm:hidden">EA</span>
        </div>

        {/* Author badge - positioned at top left */}
        {publication.author && (
          <div className="absolute top-1 left-1 sm:top-2 sm:left-2 bg-black/70 text-white text-xs px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full font-medium">
      ...
      <div className="p-2 sm:p-3 space-y-1 sm:space-y-2">
        <h3 className="text-xs sm:text-sm font-medium text-white line-clamp-2 group-hover:text-yellow-400 transition-colors duration-300 leading-tight">
          {publication.title}
        </h3>
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className="text-yellow-400/70 font-medium text-xs">
            <span className="hidden sm:inline">Evolution About</span>
            <span className="sm:hidden">EA</span>
          </span>
        </div>
